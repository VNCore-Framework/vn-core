...
function VNCore.Game.GetPedMugshot(ped, transparent)
...
function VNCore.Game.SpawnObject(object, coords, cb, networked)
    local model = type(object) == "number" and object or joaat(object)

    VNCore.Streaming.RequestModel(model)

	local obj = CreateObject(model, coords.x, coords.y, coords.z, networked == nil or networked, false, true)
	return cb and cb(obj) or obj
end

function VNCore.Game.SpawnLocalObject(object, coords, cb)
    VNCore.Game.SpawnObject(object, coords, cb, false)
end

function VNCore.Game.DeleteVehicle(vehicle)
    SetEntityAsMissionEntity(vehicle, true, true)
    DeleteVehicle(vehicle)
end

function VNCore.Game.DeleteObject(object)
    SetEntityAsMissionEntity(object, false, true)
    DeleteObject(object)
end

function VNCore.Game.SpawnVehicle(vehicleModel, coords, heading, cb, networked)
    if cb and not VNCore.IsFunctionReference(cb) then
    ...
    local playerCoords = GetEntityCoords(VNCore.PlayerData.ped)
    ...
    CreateThread(function()
        local modelHash = VNCore.Streaming.RequestModel(model)
...
function VNCore.Game.SpawnLocalVehicle(vehicle, coords, heading, cb)
    VNCore.Game.SpawnVehicle(vehicle, coords, heading, cb, false)
end

function VNCore.Game.IsVehicleEmpty(vehicle)
    return GetVehicleNumberOfPassengers(vehicle) == 0 and IsVehicleSeatFree(vehicle, -1)
end

function VNCore.Game.GetObjects()
    return GetGamePool("CObject")
end

function VNCore.Game.GetPeds(onlyOtherPeds)
    local pool = GetGamePool("CPed")

    if onlyOtherPeds then
        local myPed = VNCore.PlayerData.ped
        for i = 1, #pool do
...
function VNCore.Game.GetVehicles()
    return GetGamePool("CVehicle")
end

function VNCore.Game.GetPlayers(onlyOtherPlayers, returnKeyValue, returnPeds)
    local players = {}
    ...
    for i = 1, #active do
        ...
        if DoesEntityExist(ped) and ((onlyOtherPlayers and currentPlayer ~= VNCore.playerId) or not onlyOtherPlayers) then
...
function VNCore.Game.GetClosestObject(coords, modelFilter)
    return VNCore.Game.GetClosestEntity(VNCore.Game.GetObjects(), false, coords, modelFilter)
end

function VNCore.Game.GetClosestPed(coords, modelFilter)
    return VNCore.Game.GetClosestEntity(VNCore.Game.GetPeds(true), false, coords, modelFilter)
end

function VNCore.Game.GetClosestPlayer(coords)
    return VNCore.Game.GetClosestEntity(VNCore.Game.GetPlayers(true, true), true, coords, nil)
end

function VNCore.Game.GetClosestVehicle(coords, modelFilter)
    return VNCore.Game.GetClosestEntity(VNCore.Game.GetVehicles(), false, coords, modelFilter)
end
...
local function EnumerateEntitiesWithinDistance(entities, isPlayerEntities, coords, maxDistance)
    ...
    if coords then
        ...
        local playerPed = VNCore.PlayerData.ped
...
function VNCore.Game.GetPlayersInArea(coords, maxDistance)
    return EnumerateEntitiesWithinDistance(VNCore.Game.GetPlayers(true, true), true, coords, maxDistance)
end

function VNCore.Game.GetVehiclesInArea(coords, maxDistance)
    return EnumerateEntitiesWithinDistance(VNCore.Game.GetVehicles(), false, coords, maxDistance)
end

function VNCore.Game.IsSpawnPointClear(coords, maxDistance)
    return #VNCore.Game.GetVehiclesInArea(coords, maxDistance) == 0
end

function VNCore.Game.GetShapeTestResultSync(shape)
...
function VNCore.Game.RaycastScreen(depth, ...)
	local world, normal = GetWorldCoordFromScreenCoord(.5, .5)
	local origin = world + normal
	local target = world + normal * depth
	return target, VNCore.Game.GetShapeTestResultSync(StartShapeTestLosProbe(origin.x, origin.y, origin.z, target.x, target.y, target.z, ...))
end

function VNCore.Game.GetClosestEntity(entities, isPlayerEntities, coords, modelFilter)
...
function VNCore.Game.GetVehicleInDirection()
    local _, hit, coords, _, _, entity = VNCore.Game.RaycastScreen(5, 10, VNCore.PlayerData.ped)
    if hit and IsEntityAVehicle(entity) then
        return entity, coords
    end
end

function VNCore.Game.GetVehicleProperties(vehicle)
    ...
    return {
        ...
        plate = VNCore.Math.Trim(GetVehicleNumberPlateText(vehicle)),
        plateIndex = GetVehicleNumberPlateTextIndex(vehicle),

        bodyHealth = VNCore.Math.Round(GetVehicleBodyHealth(vehicle), 1),
        engineHealth = VNCore.Math.Round(GetVehicleEngineHealth(vehicle), 1),
        tankHealth = VNCore.Math.Round(GetVehiclePetrolTankHealth(vehicle), 1),

        fuelLevel = VNCore.Math.Round(GetVehicleFuelLevel(vehicle), 1),
        dirtLevel = VNCore.Math.Round(GetVehicleDirtLevel(vehicle), 1),
        color1 = colorPrimary,
...
function VNCore.Game.SetVehicleProperties(vehicle, props)
    if not DoesEntityExist(vehicle) then
        return
    end
    local colorPrimary, colorSecondary = GetVehicleColours(vehicle)
...
function VNCore.GetVehicleTypeClient(model)
...
VNCore.GetVehicleType = VNCore.GetVehicleTypeClient
